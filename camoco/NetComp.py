...
def accepts_iterable(fn):
    ...
    def wrapped(self, arg, *args, **kwargs):
        #
        if isinstance(arg, Iterable) and not isinstance(arg, six.string_types):
...
class NetComp(Freezable):
        ...
        with self._net_comp_buffer() as results:
            for source, target in permutations(self.networks, 2):
                ...
                for cid in source.clusters.cluster.unique():
                    ...
                    if (
                        len(cluster_genes) >= max_cluster_size
                    ...
                    if method == "density":
                        ...
                        target_coex_pval = (
                            sum(
                                [
                                    target.density(
                                        random.sample(common_genes, len(cluster_genes))
                                    )
                                    >= target_coex
                        ...
                        target_coex = target.locality(
                            cluster_genes, include_regression=True
                        ).resid.mean()
                        target_coex_pval = (
                            sum(
                                [
    ...
    def _percent_shared_mcl(self, pval_cutoff=0, comp_method="density"):
        results = []
        sig_clusters = self._query(
            f'SELECT * FROM net_comp WHERE comp_method = "{comp_method}" and target_coex_pval <= {pval_cutoff}'
        )
        sig_clusters["cluster"] = (
            sig_clusters.source + "_" + sig_clusters.source_cluster
        )
        ...
        for a, b in permutations(sig_clusters.target.unique(), 2):
            ...
            a_sig = set(
                sig_clusters.query(
                    f'target == "{a}" and source != "{a}" and source != "{b}"'
                ).cluster
            )
            b_sig = set(
                sig_clusters.query(
                    f'target == "{b}" and source != "{b}" and source != "{a}"'
                ).cluster
            )
            sig_both = set(
