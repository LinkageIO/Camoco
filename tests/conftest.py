...
def AtSeedIonome(AtTair10):
    if cf.test.force.Ontology:
        tools.del_dataset('GWAS', 'AtSeedIonome', force=True)
    if not tools.available_datasets('GWAS', 'AtSeedIonome'):
        # glob glob is god
        csvs = glob.glob(
...
def AtLeafIonome(AtTair10):
    if cf.test.force.Ontology:
        tools.del_dataset('GWAS', 'AtLeafIonome', force=True)
    if not tools.available_datasets('GWAS', 'AtLeafIonome'):
        # glob glob is god
...
def AtRootHydroIonome(AtTair10):
    if cf.test.force.Ontology:
        tools.del_dataset('GWAS','AtRootHydroIonome',force=True)
    if not tools.available_datasets('GWAS', 'AtRootHydroIonome'):
        # glob glob is god
        csvs = glob.glob(os.path.join(
            cf.options.testdir,
            'raw','GWAS','AtIonome',
